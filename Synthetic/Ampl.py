...
for delta in [0.175]:
    ...
    fig,ax   = plt.subplots(nrows=len(betas),ncols=len(sigmas),figsize=(7*len(betas),6*len(sigmas)))
    fig2,ax2 = plt.subplots(nrows=len(betas),ncols=len(sigmas),figsize=(7*len(betas),6*len(sigmas)))
    for m,beta in enumerate(betas):
        ...
        for n,sigma in enumerate(sigmas):
            ...
            for i in range(Ntraj):
                if i%1000==0: print(f"beta = {beta}, sigma = {sigma}, iteration = {i}, counter = {counter}")
            ...
            try:
                ...
                ax2[m][n].text(0.2,0.6,f"s = {sigma}",transform=ax2[m][n].transAxes)
    ...
    fig.savefig(os.path.join( out_path,f"Beta_sigma_delta_{delta}_v_x.jpg"))
    fig2.savefig(os.path.join(out_path,f"Beta_sigma_delta_{delta}_v_y.jpg"))
